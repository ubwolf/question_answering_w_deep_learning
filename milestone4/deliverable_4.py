...
class Doc2vecModel:
    ...
    def _process_text(self, text, tokens_only=False):
        """
    ...
    def fit_doc2vec(self, file_loc, col, save_file):
        ...
        tagged_data = list(self._process_text(df[col]))
    ...
    def generate_doc2vec(self):
        ...
        top_doc2vec = []
        for q in questions:
            query = list(self._process_text(q[0], tokens_only=True))
            vec = model.infer_vector(query[0])
            match = model.dv.most_similar([vec], topn=len(model.dv))
...
class QuestionAnswering:
    ...
    def __init__(self, token_model='distilbert-base-uncased-distilled-squad',
                model='distilbert-base-uncased-distilled-squad'):
        self.tokenizer = AutoTokenizer.from_pretrained(token_model, return_token_type_ids=True)
...
def load_text(file_loc, model_loc, question_loc, type='tfidf', train=False):
    if type == 'tfidf':
...
def question_answer(q_a_pairing):
    qa_model = QuestionAnswering()
    for idx, (question, response) in enumerate(q_a_pairing):
        answer = qa_model.question_answer(question, response)
        print(f"Q.{idx} {question}\n\n\tA. {answer['answer']}\t Score: {answer['score']*100:.2f}\n\nContext: {response}")
        print('------'*15)

        

if __name__ == '__main__':
    q_pair = load_text('../data/paragraph.csv',
                      '../data/doc2vec_model.model',
                      '../data/questions.txt',
                      type='tfidf', train=False)
    question_answer(q_pair)